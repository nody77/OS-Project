		...
		if(sys_isUHeapPlacementStrategyFIRSTFIT() == 1) //sys_isUHeapPlacementStrategyBESTFIT()
			{
					...
					for(index = 0 ; index < UHEAP_PAGE_ALLOCATOR_SIZE ; index+=1)
					{
						...
						{
							//cprintf("Found Block 1st if\n");
							flag_frames_found = 1;
							break;
						}
						if(user_Page_Allocation_list[index].is_full == 1)
						{
							//cprintf("Page is not free\n");
							counter = 0;
						}
						else
						{
					...
					{
						//cprintf("flag_frames_found == 1\n");
						int start_index = index - number_of_allocated_frames;
						uint32 va = (uint32)HardLimit + PAGE_SIZE + (uint32)(PAGE_SIZE * start_index);
						uint32 startVa = va;
						//cprintf("startVa : %x\n",startVa);
						for(int i = start_index ; i < (index); i += 1)
						{
							...
							else
							{
							    ...
							    user_Page_Allocation_list[i].is_full = 1;
...
void free(void* virtual_address)
{
	...
	if ((virtual_address == ((void *)HardLimit + PAGE_SIZE) || virtual_address > ((void *)HardLimit + PAGE_SIZE)) && virtual_address < (void*) USER_HEAP_MAX)
	{
		...
		for(index = 0 ; index < UHEAP_PAGE_ALLOCATOR_SIZE ; index+=1)
				...
				{
					user_Page_Allocation_list[i].is_full = 0;
					user_Page_Allocation_list[i].size = 0;

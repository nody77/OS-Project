...
int process_command(int number_of_arguments, char** arguments)
{
	...
	LIST_INIT(&foundCommands);
	int command_length_to_be_processed = strlen(arguments[0]);
	for(int i=0;i<NUM_OF_COMMANDS;i++)
	{
		int command_length = strlen(commands[i].name);
		if(command_length == command_length_to_be_processed)
		{
			if (strncmp(arguments[0],commands[i].name,command_length)==0)
			{
				...
				{
					if (commands[i].num_of_args == -1 && (number_of_arguments - 1 ) >=1)
					{
						return i;
					}
			...
			{
				int Char_found = 0;
				char * command_name_to_be_executed = arguments[0];
				char * command_name = commands[i].name;
				for(int j=0;j<command_length;j++)
				{
					for(int k=0;k<command_length;k++)
					{
						if(command_name[j] == command_name_to_be_executed[k])
						{
							Char_found = Char_found +1;
						}
					}
				}
				if(Char_found == command_length)
				{
					LIST_INSERT_TAIL(&foundCommands,&commands[i]);
				}
			}*/
		...
		else
		{
			...
			if (command_length > command_length_to_be_processed)
			{
				for(int k=0;k<command_length_to_be_processed;k++)
				{
					for(int j=temp;j<command_length;j++)
					{
						if(command_name[j] == command_name_to_be_executed[k])
						{
							//cprintf("character is found\n");
							Char_found = Char_found +1;
							temp = j;
							//cprintf("character is found  , %d \n" , Char_found);
							break;
						}

					}
				}
				if(Char_found == command_length_to_be_processed)
				{
					LIST_INSERT_TAIL(&foundCommands,&commands[i]);
				}
			}

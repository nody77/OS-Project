...
//		print_trapframe(tf);
	...
	else
	{
		if (userTrap)
		{
			...
			if((permissions & PERM_PRESENT) && (permissions & PERM_USER) != PERM_USER)
			{

				cprintf("Invalid Access 1\n");
				sched_kill_env(faulted_env->env_id);

			}
			//Pointing to unmarked page in the user heap
			if((permissions & PERM_AVAILABLE) != (PERM_AVAILABLE) && fault_va >= USER_HEAP_START && fault_va < USER_HEAP_MAX)
			{

				cprintf("Invalid Access 2\n");
				sched_kill_env(faulted_env->env_id);


			}
			//Exists but with read-only permissions
			if((permissions & PERM_PRESENT) == PERM_PRESENT && (permissions & PERM_WRITEABLE) != (PERM_WRITEABLE))
			{

				cprintf("Invalid Access 3\n");
				sched_kill_env(faulted_env->env_id);

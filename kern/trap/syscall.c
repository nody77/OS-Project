...
void* sys_uheapStart()
{
	...
	//panic("not implemented yet");
	// system call MS2
	return (void *)curenv->start;
}
void sys_env_set_nice(int niceValue)
{
	//Comment the following line before start coding...
	//panic("not implemented yet");
	// system call MS3
	env_set_nice(curenv,niceValue);
	return;
}
/**************************************************************************/
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_to_get_hardlimit:
		...
		break;
	case SYS_to_get_uheapStart:
		return (uint32)sys_uheapStart();
		break;
	case SYS_free_user_mem:
		if((void*) a1 != NULL && (a1 < USER_LIMIT - PAGE_SIZE && a1 > 0))
		{

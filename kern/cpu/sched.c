...
void clock_interrupt_handler()
{
	//TODO: [PROJECT'23.MS3 - #5] [2] BSD SCHEDULER - Your code is here
	{
		...
		if((timer_ticks()+1)%no_of_quantums_in_1sec == 0)
		{
			cprintf("IN LOAd \n");
			fixed_point_t frac1 = fix_div(fix_int(59), fix_int(60));
			fixed_point_t old_load_avg = load_avg;
			fixed_point_t temp1 = fix_mul(old_load_avg,frac1 );
			int noOfreadyprocesses = 0 ;
			for(int i = 0 ; i<num_of_ready_queues ; i++)
			{
				noOfreadyprocesses += queue_size(&(env_ready_queues[i]));
			}
			fixed_point_t frac2 =  fix_div(fix_int(1), fix_int(60));
		...
		if((timer_ticks()+1)%no_of_quantums_in_1sec == 0)
		{
			...
			fixed_point_t a = fix_div(loadAvgBy2,loadAvgBy2Plus1);// (2*load_avg) / (2*load_avg +1)
			fixed_point_t oldBYnew = fix_mul(a, old_recent_cpu);
			fixed_point_t fixed_nice = fix_int(curenv->nice);//env_get_nice(curenv)
			...
			for (int i = 0;  i < num_of_ready_queues ;i++)
			{
				...
				LIST_FOREACH(e,&(env_ready_queues[i]))
				{
					...
					fixed_point_t a = fix_div(loadAvgBy2,loadAvgBy2Plus1);// (2*load_avg) / (2*load_avg +1)
					fixed_point_t oldBYnew = fix_mul(a, old_recent_cpu);
					fixed_point_t fixed_nice = fix_int(e->nice);//env_get_nice(e)
